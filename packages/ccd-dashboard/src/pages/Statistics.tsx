...
export function Statistics() {
  ...
  const totalSessions = stats.reduce((sum: number, s: DailyStats) => sum + (s.session_count || 0), 0);
  const totalMessages = stats.reduce((sum: number, s: DailyStats) => sum + (s.message_count || 0), 0);
  const totalInputTokens = stats.reduce((sum: number, s: DailyStats) => sum + (s.total_input_tokens || 0), 0);
  const totalOutputTokens = stats.reduce((sum: number, s: DailyStats) => sum + (s.total_output_tokens || 0), 0);
  const avgSessionsPerDay = stats.length > 0 ? (totalSessions / stats.length).toFixed(1) : '0';

  const projectData = useMemo(() => {
    const filteredSessions = selectedProject
      ? allSessions.filter((s: { project_name: string | null }) => s.project_name === selectedProject)
      : allSessions.filter((s: { started_at: string }) => {
              ...
              <tbody>
                {stats.map((stat) => (
                  <tr key={stat.date} className="border-b hover:bg-gray-50 dark:hover:bg-gray-800">
                    ...
                    <td className="py-2 px-4 text-sm text-gray-900 dark:text-gray-100 text-right">{stat.session_count || 0}</td>
                    <td className="py-2 px-4 text-sm text-gray-900 dark:text-gray-100 text-right">{(stat.message_count || 0).toLocaleString()}</td>
                    <td className="py-2 px-4 text-sm text-gray-900 dark:text-gray-100 text-right">{(stat.total_input_tokens || 0).toLocaleString()}</td>
                    <td className="py-2 px-4 text-sm text-gray-900 dark:text-gray-100 text-right">{(stat.total_output_tokens || 0).toLocaleString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
